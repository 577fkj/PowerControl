...
function can_decoder(id, std_ext, data_remote, datalen, data_bin)
	...
	ERR = id_temp>>26	
			...
			ADDR = id_temp>>8	
			ret1 = ret1..string.format("|%02x", ADDR)


			-- 处理命令号
			id_temp = id_ & 0x003F0000
			CMD = id_temp>>16
			--ret1 = ret1..string.format("|命令号: %02d", CMD)
	...
	ret2 = ret2 | WIN_1
        ...
        if (ADDR==0xf0) then
          ret1 = ret1..string.format("|U:%d", (bin[1]<<24 | bin[2]<<16 | bin[3]<<8 | bin[4]))
          ret1 = ret1..string.format("|I:%d", (bin[5]<<24 | bin[6]<<16 | bin[7]<<8 | bin[8]))    
        end
      else
        ret1 = ret1..string.format("|命令号: %02d", CMD)
      end
