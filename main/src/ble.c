...
ble_gatts_t ble_gatts = {0};

void set_ble_data_callback(ble_data_callback_t callback, void *user_data)
{
    ble_gatts.callback = callback;
    ble_gatts.callback_user_data = user_data;
}

void set_ble_connected_callback(ble_connected_callback_t callback, void *user_data)
{
    ble_gatts.connected_callback = callback;
    ble_gatts.connected_user_data = user_data;
}

void set_ble_disconnect_callback(ble_connected_callback_t callback, void *user_data)
{
    ble_gatts.disconnected_callback = callback;
    ble_gatts.disconnected_user_data = user_data;
}

bool is_notify(void)
{
    return ble_gatts.is_notify;
}

bool is_connected(void)
{
    return ble_gatts.connected;
}

void diconnect(void)
{
    esp_ble_gap_disconnect(ble_gatts.esp_bd_addr);
}

bool ble_send_data(uint8_t *data, uint16_t len)
{
    if (ble_gatts.is_notify)
    {
        return esp_ble_gatts_send_indicate(ble_gatts.gatts_if, ble_gatts.conn_id, heart_rate_handle_table[IDX_CHAR_VAL_A],
                                           len, data, false) == ESP_OK;
    }
    return false;
}
...
void example_exec_write_event_env(prepare_type_env_t *prepare_write_env, esp_ble_gatts_cb_param_t *param)
{
    ...
    {
        ESP_LOGI(GATTS_TABLE_TAG, "ESP_GATT_PREP_WRITE_EXEC");
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    {
        esp_err_t set_dev_name_ret = esp_ble_gap_set_device_name(get_config()->ble_name);
        if (set_dev_name_ret)
        {
        ...
        {
            ESP_LOGI(GATTS_TABLE_TAG, "set device name success");
        }
#ifdef CONFIG_SET_RAW_ADV_DATA
    ...
    case ESP_GATTS_WRITE_EVT:
        ...
        if (heart_rate_handle_table[IDX_CHAR_CFG_A] == param->write.handle && param->write.len == 2)
        {
            ...
            if (descr_value == 0x0001)
            {
                ...
                ble_gatts.is_notify = true;
                ble_gatts.gatts_if = gatts_if;
                ble_gatts.conn_id = param->write.conn_id;
        ...
        {
            if (ble_gatts.callback)
            {
                ble_gatts.callback(param->write.value, param->write.len, ble_gatts.callback_user_data);
            }
        }
        /* send response when param->write.need_rsp is true*/
        if (param->write.need_rsp)
    ...
    case ESP_GATTS_CONNECT_EVT:
        ...
        memcpy(ble_gatts.esp_bd_addr, param->connect.remote_bda, ESP_BD_ADDR_LEN);
        ble_gatts.connected = true;
        if (ble_gatts.connected_callback)
        {
            ble_gatts.connected_callback(ble_gatts.esp_bd_addr, ble_gatts.connected_user_data);
        }
        break;
    case ESP_GATTS_DISCONNECT_EVT:
        ...
        ble_gatts.connected = false;
        if (ble_gatts.disconnected_callback)
        {
            ble_gatts.disconnected_callback(ble_gatts.esp_bd_addr, ble_gatts.disconnected_user_data);
        }
        memset(ble_gatts.esp_bd_addr, 0, ESP_BD_ADDR_LEN);
        break;
