...
static void mui_progress_bar_on_draw(mui_view_t *p_view, mui_canvas_t *p_canvas)
{
    ...
    mui_canvas_draw_utf8(p_canvas, 4, font_height - 2, string_get_cstr(p_mui_progress_bar->header));
    // mui_canvas_set_draw_color(p_canvas, 1);

    float percent =
        p_mui_progress_bar->current_value / (float)(p_mui_progress_bar->max_value - p_mui_progress_bar->min_value);
    mui_canvas_draw_rframe(p_canvas, 4, (canvas_height - font_height) / 2, canvas_width - 4, font_height, 1);
    mui_canvas_draw_box(p_canvas, 5, (canvas_height - font_height) / 2, ceil((canvas_width - 6) * percent),
                        font_height);

    char txt[32];
    uint8_t text_y = (canvas_height - font_height) / 2 + font_height * 2;
    sprintf(txt, string_get_cstr(p_mui_progress_bar->format), p_mui_progress_bar->current_value);
...
control_bar_draw_cb_t draw_cb = NULL;
void *user_data = NULL;

static void mui_progress_bar_on_enter(mui_view_t *p_view)
{
    user_data = app_control_bar_get_user_data();
    draw_cb = app_control_bar_get_draw();
    app_control_bar_set_draw(NULL, mui_progress_bar_control_view_on_draw);
}

static void mui_progress_bar_on_exit(mui_view_t *p_view)
{
    app_control_bar_set_draw(user_data, draw_cb);
}
...
mui_progress_bar_t *mui_progress_bar_create()
{
    ...
    string_init(p_mui_progress_bar->format);
    string_set_str(p_mui_progress_bar->format, "%ld");
