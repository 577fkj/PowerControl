...
uint8_t mui_canvas_draw_utf8_right(mui_canvas_t *p_canvas, uint8_t x, uint8_t y, const char *str)
{
    if (!str)
        return 0;
    x += p_canvas->offset_x;
    y += p_canvas->offset_y;
    x -= mui_canvas_get_utf8_width(p_canvas, str);
    return u8g2_DrawUTF8(p_canvas->fb, x, y, str);
}
...
uint8_t mui_canvas_draw_glyph(mui_canvas_t *p_canvas, uint8_t x, uint8_t y, uint16_t encoding)
{
    x += p_canvas->offset_x;
    y += p_canvas->offset_y;
    return u8g2_DrawGlyph(p_canvas->fb, x, y, encoding);
}
