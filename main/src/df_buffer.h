...
static inline void buff_put_float(buffer_t *buffer, float value)
{
    *(float *)(&buffer->buff[buffer->limit]) = value;
    buffer->limit += 4;
}
...
static inline void buff_get_string(buffer_t *buffer, char *string, size_t max_length)
{
    ...
    else
    {
        ...
        buff_get_byte_array(buffer, (uint8_t *)string, min_length);
        string[min_length] = '\0';
...
static inline void buff_set_limit(buffer_t *buffer, uint32_t limit) { buffer->limit = limit; }
